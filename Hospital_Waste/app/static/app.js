...
async function loadFacilities(){
  ...
  data.forEach(f=>{
    ...
    opt.value = f.facility_code || f.facility_id || '';
...
const els = {
  ...
  trendSummaryPill: document.getElementById("trendSummaryPill"),
};
...
function renderExecutiveSummary(summary){
  ...
  if(!summary){
    el.innerHTML = '<div class="card">Summary verisi yok</div>';
    return;
  }

  const riskColor =
    summary.risk === "high" ? "#D51635" :
    summary.risk === "medium" ? "#f9ba00" :
    "#22c55e";

  el.innerHTML = `
    <div class="card" style="padding:18px;border:1px solid rgba(255,255,255,.08);border-radius:16px;background:linear-gradient(180deg,rgba(12,23,42,.9),rgba(8,18,37,.85));">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
        <div style="font-size:13px;color:#8fa3bf;">Executive Score</div>
        <div style="font-size:12px;color:${riskColor};font-weight:700;">${String(summary.risk || "").toUpperCase()}</div>
      </div>
      <div style="font-size:34px;font-weight:800;color:#fff;">${Math.round(summary.score || 0)}</div>
    </div>

    <div class="card" style="padding:18px;border:1px solid rgba(255,255,255,.08);border-radius:16px;background:rgba(213,22,53,.06);">
      <div style="font-size:12px;color:#8fa3bf;">Top Issue</div>
      <div style="font-size:15px;font-weight:600;margin-top:6px;color:#fff;">
        ${summary.top_issue || "—"}
      </div>
    </div>

    <div class="card" style="padding:18px;border:1px solid rgba(255,255,255,.08);border-radius:16px;background:rgba(34,197,94,.06);">
      <div style="font-size:12px;color:#8fa3bf;">Quick Action</div>
      <div style="font-size:15px;font-weight:600;margin-top:6px;color:#fff;">
...
function drawDailyTrendSVG(svg, data){
  ...
  const values = data.map(d => d.co2_kg || 0).reverse();
...
svg.setAttribute("viewBox", `0 0 ${w} ${h}`);
  svg.innerHTML = `
    <defs>
      <linearGradient id="trendFillGradient" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#f9ba00" stop-opacity="0.22"/>
        <stop offset="100%" stop-color="#f9ba00" stop-opacity="0.02"/>
      </linearGradient>
    </defs>
    <path d="${areaPath}" fill="url(#trendFillGradient)" stroke="none"/>
    <path d="${path}" fill="none" stroke="#f9ba00" stroke-width="3"/>
  `;
  ...
  let label = "→ Stable";
  if(last > first * 1.03) label = "↑ Increasing";
  else if(last < first * 0.97) label = "↓ Improving";

  const pill = document.getElementById("trendSummaryPill");
...
async function loadAll(facilityCode){

  const selectedFacility =
    facilityCode || document.getElementById("facilitySelect")?.value;

  const ai = await safeFetch(`/api/ai/summary?facility_code=${selectedFacility}&days=14`);
  const latestRows = await safeFetch(`/api/dashboard/latest?facility_code=${selectedFacility}`);
  const daily = await safeFetch(`/api/dashboard/daily?facility_code=${selectedFacility}&days=14`);
  ...
  renderExecutiveSummary(latest?.sustainability_scoring?.summary);

  drawTrend(els.trendChart, daily);
  drawImpact(els.impactChart, daily);
  drawDailyTrendSVG(els.dailyTrendChart, daily);
  ...
  daily.forEach(r=>{
    ...
    tr.innerHTML=`
      ...
      <td>${r.facility}</td>
      <td>${((r.medical_waste_kg||0)+(r.pathological_waste_kg||0)+(r.general_waste_kg||0)).toFixed(1)}</td>
      <td>${fmt(r.energy_kwh)}</td>
      <td>${fmt(r.water_m3)}</td>
      <td>${(r.scope2_location_tco2e ? (r.scope2_location_tco2e*1000).toFixed(0) : '—')}</td>
      <td style="color:#22c55e;font-weight:bold">LIVE ●</td>
    `;
    els.dailyTableBody.appendChild(tr);
  });

  await loadClinicDashboard(selectedFacility);
}
...
async function boot(){
  ...
  els.refreshBtn.onclick = async () => {
    ...
    try {
      await loadAll(document.getElementById("facilitySelect")?.value );
...
async function loadClinicDashboard(facility){
  const selectedFacility = facility || document.getElementById("facilitySelect")?.value;

  const highlights = await safeFetch(`/api/dashboard/clinic-highlights?facility_code=${selectedFacility}`);
  const daily = await safeFetch(`/api/dashboard/clinic-daily?facility_code=${selectedFacility}&days=14`);
...
function bindHospitalIntakeForm(){
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const payload = {
      facility_code: document.getElementById("facilitySelect")?.value || null,
