...
def dashboard_clinic_highlights(
    ...
    facility_code = resolve_facility_code(None, facility_code)
    ...
    allowed_sort = {
        "rank": "highlight_sirasi",
        "risk_score": "risk_score",
        "clinic": "klinik_adi",
        "risk_level": "risk_seviyesi",
    }
    order_col = allowed_sort.get(sort_by, "highlight_sirasi")
    order_dir = "DESC" if str(sort_dir).lower() == "desc" else "ASC"
    ...
    sql = f"""
        ...
        WHERE LOWER(h.facility_name) = LOWER(
            CASE
                WHEN LOWER(%s) = 'bazekol' THEN 'Bazekol Çiğli Hastanesi'
                ELSE %s
            END
        )
        ...
        LIMIT %s
    """
    rows = fetch_all(sql, (facility_code, facility_code, limit))
    return rows
...
def dashboard_clinic_daily(
    ...
    order_col = allowed_sort.get(sort_by, "clinic")
    order_dir = "DESC" if str(sort_dir).lower() == "desc" else "ASC"
    ...
    sql = f"""
        ...
        WHERE LOWER(d.facility_code) = LOWER(
            CASE
                WHEN LOWER(%s) = 'bazekol' THEN 'CIGLI_HOSP'
                ELSE %s
            END
        )
        ...
        ORDER BY {order_col} {order_dir}, clinic ASC
    """
    rows = fetch_all(sql, (facility_code, facility_code, clinic, clinic, days))
    return rows
